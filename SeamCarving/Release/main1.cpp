...
void filter(int h, int w, float** gradient)
{
	for (int i = 0; i < h; i++)
	{
		for (int j = 0; j < w; j++)
		{
			if (gradient[i][j] < 15) gradient[i][j] = 0.0;
		}
	}
}
...
int main(int argc, char** argv)
{
	...
	for (;;)
	{
		...
		// Stage 1
		if ((current_time - last_change > 3000) && (stage==1))
		{
			to_Gradient(grayscale, img_h, img_w, gradient);
		...
		// Stage 2
		/*if ((current_time - last_change > 6000) && (stage == 2))
		{
			if (current_time - last_change > 12000) stage++;
			to_Gradient(grayscale, img_h, img_w, gradient);
			...
			for (int i = 0; i < img_h; i++)
			{
				for (int j = 0; j < img_w; j++)
				{
					float tmp = (blue[i][j] + green[i][j] + blue[i][j] + red[i][j]) / 3;
					pixels[index++] = tmp;
					pixels[index++] = tmp;
					pixels[index++] = tmp;
				}
		...
		if ((current_time - last_change > 6000) && (stage == 2))
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				filter(img_h, img_w, gradient);
				float lowest = get_Least_Important_Column(gradient, img_h, img_w, pos);
				if ((current_time - last_change > 18000) || (lowest > avg / 4))
				{
					stage++;
					break;
				}
